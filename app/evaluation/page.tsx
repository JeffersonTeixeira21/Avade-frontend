...
import type { Teacher } from '@/lib/firestoreService';
import { getTeachers, addEvaluation } from '@/lib/firestoreService';
...
export default function EvaluationPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const allAnswered = Array.from({ length: 8 }, (_, i) => `q${i + 1}`).every(key => objectiveScores[key] !== undefined);
    ...
    try {
      ...
      setToastMessage('Erro ao enviar avaliação. Tente novamente.');
      setToastVisible(true);
    } finally {
      setSubmitting(false);
    }
    ...
    <div className="bg-white p-8 rounded-xl shadow-xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <button
          type="submit"
          disabled={submitting}
          className={`w-full py-3 px-4 bg-green-600 text-white font-bold rounded-lg hover:bg-green-700 ${submitting ? 'opacity-60 cursor-not-allowed' : ''}`}
        >
